...
$reportJs=file_get_contents($root.'/public/assets/reporting-workspace.js');
...
foreach(['Collecter les KPI Meta','Importer l’historique d’un réseau','Messages et commentaires','reporting-client','reporting-page','publication-link']as$needle)if(!str_contains($view,$needle))throw new RuntimeException('Reporting AJAX incomplet: '.$needle);
if(!str_contains($reportJs,"credentials: 'same-origin'"))throw new RuntimeException('Reporting AJAX incomplet: credentials same-origin');
...
foreach(["class_exists('TCPDF')",'renderWithTcpdf','dejavusans','outputCalendarFallback']as$needle)if(!str_contains($pdf,$needle))throw new RuntimeException('Rendu PDF TCPDF incomplet: '.$needle);
if(!str_contains($config,'app/third_party/tcpdf/tcpdf.php')||!is_file($root.'/app/third_party/tcpdf/tcpdf.php'))throw new RuntimeException('Runtime TCPDF embarqué absent.');
