...
$pdo=Database::getConnection();$connectionId=0;$publicationId=0;
try {
    ...
    $clientStmt=$pdo->prepare("SELECT id FROM clients WHERE tenant_id=:tenant AND statut='Actif' ORDER BY id LIMIT 1");$clientStmt->execute(['tenant'=>$tenantId]);$clientId=(int)$clientStmt->fetchColumn();if(!$clientId)throw new RuntimeException('Aucun client actif dans le tenant de test.');
    ...
    $model=new SocialPublishingModel();$publicationId=$model->createPublication(['client_id'=>$clientId,'connection_ids'=>[$connectionId],'master_title'=>'Regression Meta','master_caption'=>'Publication technique non envoyée au réseau.','publish_mode'=>'Scheduled','scheduled_at'=>date('Y-m-d H:i:s',time()+86400)],[],(int)$user['id']);
...
} catch(Throwable $exception) { fwrite(STDERR,get_class($exception).': '.$exception->getMessage().PHP_EOL); exit(1); }
