...
class SocialPublishingController extends Controller {
    ...
    public function index(){
        if($this->isPost()){$this->requirePermission('publishing.manage');try{$action=(string)($_POST['action']??'');$user=(int)$this->currentUser()['id'];
        ...
        $data=$this->model->dashboardData();$this->render('social-publishing/index',array_merge($data,['pageTitle'=>'Publication multiréseau','clients'=>$this->model->clients(),'providers'=>SocialPublishingModel::PROVIDERS,'canManage'=>$this->can('publishing.manage'),'canApprove'=>$this->can('publishing.approve')]));
    }
}