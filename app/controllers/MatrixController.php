...
class MatrixController extends Controller {
    ...
    private function addIdea(array $matrix, $projectId, $month) {
        if ($projectId>0) { TenantGuard::assertProject($projectId); $this->assertProjectClient($projectId,(int)$matrix['client_id']); }
        ...
        $stmt->execute(['matrix'=>(int)$matrix['id'],'tenant'=>$this->tenantId,'client'=>(int)$matrix['client_id'],'project'=>$projectId?:null,'month'=>$projectId>0?$month.'-01':null,'target'=>trim((string)($_POST['target_audience']??'')),'objective'=>trim((string)($_POST['objective']??'')),'problem'=>trim((string)($_POST['problem_need']??'')),'product'=>trim((string)($_POST['product_offer']??'')),'format'=>trim((string)($_POST['creative_format']??$matrix['default_format'])),'type'=>$type,'platform'=>trim((string)($_POST['platform']??'')),'hook'=>$hook,'cta'=>trim((string)($_POST['call_to_action']??'')),'brief'=>$brief,'script'=>$script,'priority'=>in_array($_POST['priority']??'', ['Haute','Moyenne','Basse'],true)?$_POST['priority']:'Moyenne','user'=>(int)($this->currentUser()['id']??0)?:null]);
    ...
    private function generateCombinations(array $matrix, $projectId, $month) {
        ...
        for ($i=0; $i<$count; $i++) {
            ...
            $insert->execute(['matrix'=>(int)$matrix['id'],'tenant'=>$this->tenantId,'client'=>(int)$matrix['client_id'],'project'=>$projectId?:null,'month'=>$projectId>0?$month.'-01':null,'target'=>$target,'objective'=>$objective,'problem'=>$problem,'product'=>$product,'format'=>$format,'type'=>$matrix['default_deliverable_type'],'platform'=>$platform,'hook'=>$hook,'cta'=>$cta,'brief'=>$brief,'script'=>$script,'anchor_type'=>$anchorType,'anchor_value'=>$anchorValue,'user'=>(int)($this->currentUser()['id']??0)?:null]);
        }
    ...
    private function syncIdeas(array $matrix,$projectId,$month) {
        ...
        try {
            foreach($ideas as $idea){
                ...
                $this->pdo->prepare("UPDATE matrix_ideas SET projet_id=:project,target_month=:month,status='Synchronisee',synced_deliverable_id=:deliverable WHERE id=:id")->execute(['project'=>$projectId,'month'=>$month.'-01','deliverable'=>$deliverableId,'id'=>$idea['id']]);
            }
    ...
    private function ideasForContext($matrixId,$projectId,$month){$sql='SELECT * FROM matrix_ideas WHERE matrix_id=:matrix AND tenant_id=:tenant AND synced_deliverable_id IS NULL';$params=['matrix'=>$matrixId,'tenant'=>$this->tenantId];if($projectId>0){$sql.=' AND (projet_id=:project OR projet_id IS NULL)';$params['project']=$projectId;}else{$sql.=' AND projet_id IS NULL';}$sql.=' ORDER BY id DESC';$stmt=$this->pdo->prepare($sql);$stmt->execute($params);return$stmt->fetchAll(PDO::FETCH_ASSOC);}
