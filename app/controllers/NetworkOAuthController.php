...
class NetworkOAuthController extends Controller {
    ...
    private function save(array $connection,array $account,array $tokens): void {
        ...
        try {
            ...
            $stmt->execute(['external'=>$account['id'],'label'=>$account['name'],'type'=>$account['type'],'access'=>$access,'refresh'=>$refresh??($current['refresh_token_encrypted']??null),'expires'=>$expires,'scopes'=>json_encode($scopes),'metadata'=>json_encode(['oauth_connection_only'=>true,'analytics_enabled'=>false,'publishing_enabled'=>false]),'user'=>(int)$this->currentUser()['id'],'id'=>$connection['id'],'tenant'=>TenantGuard::tenantId()]);
            $pdo->commit();
