...
class SocialInboxController extends Controller {
    ...
    public function index(){ $connections=$this->service->connections(TenantGuard::tenantId());$selected=(int)($_GET['connection_id']??($connections[0]['id']??0));$inbox=['posts'=>[],'conversations'=>[]];$error=null;if($selected)try{$inbox=$this->service->inbox($selected,TenantGuard::tenantId());}catch(Throwable$e){$error=$e->getMessage();}$this->render('social-inbox/index',compact('connections','selected','inbox','error')+['pageTitle'=>'Messages et commentaires','canReply'=>$this->can('publishing.manage')]); }
