...
class PdfExportService {
    ...
    private static function buildBriefsHtml(string $title, array $rows): string {
        ...
        <?php $shown=[]; foreach ([['plan_script','Plan de tournage'],['texte_script','Script'],['script_contenu','Script']] as [$key,$label]): $value=trim((string)($row[$key]??'')); $fingerprint=preg_replace('/\s+/u',' ',mb_strtolower($value)); if($value!==''&&!isset($shown[$fingerprint])): $shown[$fingerprint]=true; ?><div class="block"><strong><?= $label ?></strong><?= nl2br(htmlspecialchars($value), false) ?></div><?php endif; endforeach ?></section><?php endforeach ?><?php if (!$rows): ?><div class="empty">Aucun brief ou script dans la sélection.</div><?php endif ?><div class="footer">Strax · Document de production</div></body></html>
        <?php return (string)ob_get_clean();
    }
