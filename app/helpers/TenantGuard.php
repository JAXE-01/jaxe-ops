...
class TenantGuard {
    ...
    private static function assertProjectAssignee($userId,$clientId){
        ...
        if($organizationIds){$placeholders=implode(',',array_fill(0,count($organizationIds),'?'));$stmt=Database::getConnection()->prepare("SELECT 1 FROM tenant_memberships WHERE user_id=? AND organization_id IN ($placeholders) AND status IN ('Actif','Active') LIMIT 1");$stmt->execute(array_merge([$userId],$organizationIds));if($stmt->fetchColumn())return;}
        throw new RuntimeException('Le responsable sélectionné ne fait pas partie de l’agence qui gère ce client.');
    }
    ...
    private static function currentOrganizationId($tenantId){$stmt=Database::getConnection()->prepare("SELECT organization_id FROM tenant_memberships WHERE tenant_id=:tenant_id AND user_id=:user_id AND status IN ('Actif','Active') LIMIT 1");$stmt->execute(['tenant_id'=>$tenantId,'user_id'=>(int)($_SESSION['user']['id']??0)]);return (int)($stmt->fetchColumn()?:0);}
    ...
    private static function userBelongsToTenant($id,$tenantId){$stmt=Database::getConnection()->prepare("SELECT 1 FROM tenant_memberships WHERE user_id=:user_id AND tenant_id=:tenant_id AND status IN ('Actif','Active') LIMIT 1");$stmt->execute(['user_id'=>$id,'tenant_id'=>$tenantId]);return(bool)$stmt->fetchColumn();}
