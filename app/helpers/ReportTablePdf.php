...
class ReportTablePdf {
    public static function html(string $title,array $rows,array $columns): string {
        ...
        $legend='';
        foreach(['vues','likes','commentaires','partages','clics'] as $metric)$legend.=ReportIcons::pdf($metric).' '.$esc(ReportPresentation::label($metric)).' &nbsp; ';
        $html='<html><head><meta charset="UTF-8"><style>body{font-family:dejavusans;color:#23364d;font-size:9pt}h1{font-size:18pt;color:#183453}h2{font-size:11pt}th{background-color:#eaf0f5;color:#23364d;font-weight:bold}td{border-bottom:0.4pt solid #dce4ed}a{color:#246597;text-decoration:none}</style></head><body><h1>'.$esc(PdfReportText::normalize($title)).'</h1><p style="color:#687b90">STRAX / RAPPORT DE PERFORMANCE - '.count($rows).' ligne(s)<br>'.$legend.'<br>↓ Importée - ↑ Publiée via Strax - Donnée absente : —<br>Alphabets décoratifs normalisés. Les symboles non couverts sont explicités entre crochets ; le texte original reste accessible via le lien.</p>';
