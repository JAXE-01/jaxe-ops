...
<details class="panel field-wide cadence-builder" <?= $cadenceRows?'open':'' ?>>
 ...
 <div class="cadence-builder-body">
  ...
  <?php if(!empty($record['id'])): ?><div class="cadence-revision"><label><span>Appliquer à partir de</span><input type="month" name="cadence_effective_month" min="<?= date('Y-m') ?>" value="<?= htmlspecialchars((string)($_POST['cadence_effective_month']??date('Y-m'))) ?>"></label><label class="cadence-confirm"><input type="checkbox" name="cadence_confirm_future" value="1"><span>Appliquer au mois choisi et aux suivants. Les contenus déjà programmés, publiés ou travaillés restent inchangés.</span></label></div><?php endif ?>
