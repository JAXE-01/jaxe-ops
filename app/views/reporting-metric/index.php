...
<section class="panel">
    ...
    <div class="social-reporting-actions">
        ...
        <details>
            <summary class="report-icon-button" title="Importer l’historique d’un réseau" aria-label="Importer l’historique d’un réseau"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 4v11m0 0 4-4m-4 4-4-4M5 19h14"/></svg></summary>
            <form method="post" action="<?= htmlspecialchars(route_url('/reporting-metric/import-social-history')) ?>" class="social-history-form">
                <label>Compte ou page<select name="connection_id" required><option value="">Choisir</option><?php foreach($socialConnections as $connection):if(($connection['status']??'')!=='Connected'||!in_array($connection['provider']??'',['facebook','instagram','youtube','tiktok','linkedin'],true))continue;$provider=(string)$connection['provider'];$scopes=(array)json_decode((string)($connection['scopes_json']??'[]'),true);$eligible=$provider!=='linkedin'&&($provider!=='tiktok'||in_array('video.list',$scopes,true));?><option value="<?= (int)$connection['id'] ?>" <?= $eligible?'':'disabled' ?>><?= htmlspecialchars(($connection['account_label']??'Compte').' · '.ucfirst($provider).(!$eligible?' · autorisation de collecte requise':'')) ?></option><?php endforeach?></select></label>
